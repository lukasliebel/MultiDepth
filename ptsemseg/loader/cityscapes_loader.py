...
class cityscapesLoader(data.Dataset):
    ...
    def __init__(
        ...
        self.n_classes_semseg = 19
        self.n_classes = self.n_classes_semseg  # HACK
        self.img_size = (
    ...
    def transform(self, img, lbl):
        ...
        if not np.all(classes == np.unique(lbl)):
            logger.debug("Resizing labels yielded fewer classes")

        if not np.all(np.unique(lbl[lbl != self.ignore_index]) < self.n_classes):
            logger.error("after det", classes, np.unique(lbl))
