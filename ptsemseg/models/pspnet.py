...
class pspnet(nn.Module):
    ...
    def __init__(
        ...
        self.n_classes_d_cls = (
            n_classes_d_cls if n_classes_d_cls is not None else 21
        )
        self.input_size = (
        ...
        # Head for d_cls
        if d_cls:
            self.head_d_cls_pyramid_pooling = pyramidPooling(2048, [6, 3, 2, 1])
            self.head_d_cls_cbr_final = conv2DBatchNormRelu(4096, 512, 3, 1, 1, False)
            self.head_d_cls_dropout = nn.Dropout2d(p=0.1, inplace=False)
            self.head_d_cls_classification = nn.Conv2d(512, self.n_classes_d_cls, 1, 1, 0)

        # Head for d_reg
        if d_reg:
            self.head_d_reg_pyramid_pooling = pyramidPooling(2048, [6, 3, 2, 1])
            self.head_d_reg_cbr_final = conv2DBatchNormRelu(4096, 512, 3, 1, 1, False)
            self.head_d_reg_dropout = nn.Dropout2d(p=0.1, inplace=False)
            self.head_d_reg_classification = nn.Conv2d(512, 1, 1, 1, 0)

        # NOTE Not using auxiliary loss of original PSPnet architecture
        ...
        # )
        # self.aux_cls = nn.Conv2d(256, self.n_classes_d_cls, 1, 1, 0)
        #
    ...
    def forward(self, x):
        ...
        if self.d_cls:
            ...
            x_d_cls = self.head_d_cls_pyramid_pooling(x)

            x_d_cls = self.head_d_cls_cbr_final(x_d_cls)
            x_d_cls = self.head_d_cls_dropout(x_d_cls)

            x_d_cls = self.head_d_cls_classification(x_d_cls)
            x_d_cls = F.interpolate(x_d_cls, size=inp_shape, mode="bilinear", align_corners=True)

            outputs["d_cls"] = x_d_cls

        # Add head for d_reg
        if self.d_reg:

            x_d_reg = self.head_d_reg_pyramid_pooling(x)

            x_d_reg = self.head_d_reg_cbr_final(x_d_reg)
            x_d_reg = self.head_d_reg_dropout(x_d_reg)

            x_d_reg = self.head_d_reg_classification(x_d_reg)
            x_d_reg = F.interpolate(x_d_reg, size=inp_shape, mode="bilinear", align_corners=True)

            outputs["d_reg"] = x_d_reg
