...
class PolynomialLR(_LRScheduler):
    ...
    def get_lr(self):
        if not (self.last_epoch % self.decay_iter or self.last_epoch % self.max_iter):
            # print("lr was not adapted and still is", [base_lr for base_lr in self.base_lrs])
            return [base_lr for base_lr in self.base_lrs]
        else:
            factor = (1 - self.last_epoch / float(self.max_iter)) ** self.gamma
            # print("lr was adapted and is now", [base_lr * factor for base_lr in self.base_lrs])
